...
int main(int argc, char** argv){
    ...
    Liste_tableau_bezier3 liste_tab_s;
    Cellule_Tableau_bezier3 *cell_s;
    Liste_bezier3 liste_be3;
    Tableau_bezier3 tab;
    int i, nb_segment;
    Image image;
    char* nom_fichier;
    ...
    for(i=0;i<liste_tab.taille;i++){
        liste_be3 = simplification_bezier_3(cell->data,0,cell->data.taille-1,atof(argv[3]));
        // printf("sortie de simplification\n");
        tab = sequence_bezier3_liste_vers_tableau(liste_be3);
        // printf("tableau créer\n");
        // printf("taille tab post simplification n°%d: %d\n",i+1, tab.taille);
        nb_segment += tab.taille;
        cell_s = creer_element_Tableau_bezier3(tab);
        // printf("elt tableau\n");
        cell_s->suiv = NULL;
        cell_s->data = tab;
    ...
    eps_bezier3(liste_tab_s,nom_fichier,image,atoi(argv[2]));
